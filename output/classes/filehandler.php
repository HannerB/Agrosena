...
class RunnerFileHandler {
	...
	protected function validateFile( $filename, $fileSize, &$errorMessage ) {
        if ( !$filename ) {
            $errorMessage = "No se ha indicado el nombre de fichero";
            return false;
        }
		...
		if( $acceptedTypes ) {
			...
			if( array_search( $ext, $acceptedTypes ) === false && array_search( ".".$ext, $acceptedTypes ) === false ) {
				$errorMessage = "El tipo de fichero es erróneo";
				return false;
			}
		}
		...
		if ( $maxFileSize && $fileSize > $maxFileSize * 1024 ) {
            $errorMessage = mysprintf("El tamaño del fichero supera el límite de %s KBytes", array( $maxFileSize ) );
            return false;
        }
		...
		if ( $maxTotalFileSize ) {
			if( $this->getUploadFilesSize() + $fileSize > $maxTotalFileSize * 1024 ) {
				$errorMessage = mysprintf( "El tamaño de los ficheros supera el límite de %s KBytes", array( $maxTotalFileSize ) );
				return false;
			}
		}
		...
		if ( $maxNumberOfFiles && $this->getUploadFilesCount() >= $maxNumberOfFiles ) {
			$errorMessage = mysprintf( "No puede enviar más de %s ficheros", array( $maxNumberOfFiles ) );
			return false;
		}
        return true;
	}
